...
void ParticleEmitter::spawn(float time) {
	...
	switch (type) {
        ...
        case RingEmitter:  {
                ofVec3f dir = ofVec3f(ofRandom(-1.0, 1.0), 0.0 , ofRandom(-1.0, 1.0));
                float speed = velocity.length();
                dir = dir.getNormalized();
                particle.velocity = ofVec3f(dir.x * speed , ofRandom(-ringEmmiterHeight/2, ringEmmiterHeight/2) , dir.z * speed);
                particle.position.set(position);
            }
            break;
	}
	...
	particle.radius = particleRadius + ofRandom(particleRadiusVariance);
	particle.mass = mass;
